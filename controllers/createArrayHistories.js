...
module.exports = async (req, res) => {
  ...
  if (!Array.isArray(histories)) {
    return res.status(400).json({
      success: false,
      message: "Se espera un array de historias clínicas"
    });
  }
  ...
  for (const entry of histories) {
    ...
    try {
      ...
      // Crear la historia clínica con el patient_id y cedula
      const medicalHistory = await MedicalHistory.create({
        cedula,
        patient_id: patient.id
      });

      // Crear las secciones relacionadas
      if (clinical_data) {
      ...
      // Crear fechas de evolución si existen
      if (Array.isArray(evolutions) && evolutions.length > 0) {
        const evolutionsPrepared = evolutions.map((e) => ({
          ...e,
          medical_history_id: medicalHistory.id
        }));
        await EvolutionDate.bulkCreate(evolutionsPrepared);
      }
